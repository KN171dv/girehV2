...
Composition is a 12-column grid at lg that is asymmetric by default and deliberately broken at the edges. The manifesto runs text in columns 1–7 with its figure in 8–12 bleeding past the gutter (`-mr-6 / -mr-10 / -mr-14`). The craft section abandons flow at xl entirely: a `min-h-[46rem]` stage with a 41%-wide figure anchored left, text anchored right, and a second 28%-wide figure absolutely placed at `left-[24%]` crossing over the first, separated from it by a 10px grafite border. Below xl the section is a vertical sequence — figure, text, figure — where the second figure is right-aligned and the first pulls back to 84% from md up, so the two read as offset plates at different horizontal positions rather than as one crossing the other. The threshold sits at xl, not lg, because 1024px is a tablet in landscape and a phone in desktop-site mode: there the crossing has no room and reads as a layout collision rather than as depth (corrected 2026-09-19 on client report). The services section pairs a sticky image panel (columns 1–5, `top-28`) with the ledger (columns 6–12). The barber roster is a 1 → 2 → 4 column grid whose four columns carry staggered top offsets (0, 5rem, 2rem, 7rem) so the row never reads as a card shelf. The gallery is a full-bleed horizontal strip of six panels, four aspect ratios and per-panel vertical offsets (±1.5rem–4rem), sized at lg by `min(vw, svh)` so it fits short screens.
