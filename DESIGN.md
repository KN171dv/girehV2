...
- A platform-split motion system: scrubbed parallax and one pin on desktop pointers; one-shot reveals on touch; nothing hidden under reduced motion.
...
- **Osso** (#ede6d6): the primary text colour on every surface — headlines, body, labels — and the sole text colour on the solid latão button's bordô hover state. Secondary copy steps down by opacity (osso/85, /75, /70, /65, /55, /45) rather than by a second grey.
...
**The Opacity-Not-Grey Rule.** Secondary and tertiary text is osso at reduced alpha, never a new neutral. The palette stays at six colours no matter how many tiers of copy a section needs.
...
- **Title-sm** (400, 1.35rem): the footer's "Onde / Quando / Contato" heads; the header wordmark sits just above at 1.4rem.
...
- **Micro** (400, 0.82rem / 0.78rem): hero meta (address, hours), service durations, the quiet booking link, the footer's closing line (0.82); gallery captions (0.78) — the floor of the ramp.
...
Composition is a 12-column grid at lg that is asymmetric by default and deliberately broken at the edges. The manifesto runs text in columns 1–7 with its figure in 8–12 bleeding past the gutter (`-mr-6 / -mr-10 / -mr-14`). The craft section abandons flow at lg entirely: a `min-h-[40rem]` stage with a 46%-wide figure pinned left, text pinned right, and a second 40%-wide figure absolutely placed at `left-[30%]` crossing over the first. The services section pairs a sticky image panel (columns 1–5, `top-28`) with the ledger (columns 6–12). The barber roster is a 1 → 2 → 4 column grid whose four columns carry staggered top offsets (0, 5rem, 2rem, 7rem) so the row never reads as a card shelf. The gallery is a full-bleed horizontal strip of seven panels at seven different widths (15vw–32vw at lg), four aspect ratios and per-panel vertical offsets (±2.5rem–4rem).

Responsive behaviour is a genuine fork, not a reflow: at lg the gallery becomes a pinned `h-screen` stage scrubbed horizontally; below lg it is a native `overflow-x-auto` snap strip. The services image panel exists only at lg; below it, each row grows its own 16:9 image band.

## Elevation & Depth

Flat by doctrine. Depth is built from alternating carvão/grafite bands, carvão gradient veils and vignettes stacked over photography (`from-carvao via-carvao/45 to-carvao/5`, a radial vignette, and a scroll-darkened veil in the hero), overlapping figures, and parallax layers moving at different speeds. Nothing at rest carries a shadow.
...
- **Solid:** latão fill, carvão label, 28px/16px padding. On hover and `:focus-visible` a bordô layer scales up from the bottom over 420ms `cubic-bezier(0.16,1,0.3,1)` and the label transitions to osso.
...
### Navigation
Fixed, full-width, transparent over the hero with a 7rem carvão/75 → transparent gradient scrim keeping the wordmark and links legible against the bright facade. After 60% of a viewport of scroll it condenses over 500ms into a carvão/85 backdrop-blurred bar with a bronze/30 hairline and reduced padding, and the scrim fades out. Links are Archivo 0.9rem at osso/75 with a latão underline that wipes in from the left on hover; the wordmark is the display serif at 1.4rem. Below lg the nav collapses to a two-line hamburger that rotates into an X, opening a full-screen carvão panel whose links (display serif, 2rem, bronze/25 dividers) stagger in at 70ms intervals; the panel locks body scroll, closes on Escape, and manages `tabIndex` so hidden links stay out of the tab order.

### The Services Ledger (signature component)
An editorial price sheet, not a menu of cards: a single `<ul>` with a bronze/35 rule on top and one per row. Each row sets the service name in the display serif (1.5 → 2rem) opposite its price in the same face in latão, with duration (0.82rem, osso/55) and a quiet booking link sharing a line beneath the name. Hovering or focusing a row tints the name latão and drives the sticky image panel beside it, where the seven service photographs cross-fade over 700ms with a 1.05 → 1.00 scale settle. Below lg the sticky panel disappears and each row grows its own full-width 16:9 image band with a carvão gradient foot.

### The Barber Roster (signature component)
Four bare grayscale portraits (aspect 3/4) with no frame, no border and no card: the image, a name in the display serif at 1.6rem, a note at 0.92rem osso/65, and a full-width outline booking button pre-filled with that barber's name. A grafite/70 gradient rises from the foot of each portrait; hovering scales the image 1.04 over 900ms. The four columns carry staggered top offsets so the group reads as a composition rather than a row of tiles. Grayscale is deliberate and total — it keeps portrait lighting and skin tone from acting as a seventh colour.

### The Gallery Strip (signature component)
Seven panels of mismatched widths, aspect ratios and vertical offsets, each with a 0.78rem caption at osso/45. At lg the section becomes the site's only pinned stage: it fills `h-screen`, ScrollTrigger pins it, and the track translates horizontally by `scrollWidth - innerWidth + 96` under a scrub of 1, re-measuring on image load and refresh. Below lg it is a native horizontal scroll container with `snap-x snap-mandatory`, a hidden scrollbar and one-shot staggered reveals.
...
- **Desktop** (`min-width: 1024px` AND `pointer: fine` AND no reduced-motion preference): Lenis smooth wheel scrolling (duration 1.05, `syncTouch: false`) bridged into ScrollTrigger; scrubbed parallax with a different speed per layer (±6% and ±11% on the craft figures, ±5–11% across the barber portraits, ±8% on the manifesto figure, ±7% plus a 1.08 → 1.14 scale drift on the footer band); the hero's image scales 1.16 → 1.18 while its content recedes to 15% opacity and its veil darkens; and one pinned horizontal section.
- **Touch** (all coarse pointers, including wide tablets): one-shot reveals only. No pin, no scrub, no Lenis, no scroll hijack. Verified by touch-event testing: zero pin-spacers, no scroll regressions, no drift after the finger lifts.
...
**The One-Pin Rule.** The gallery is the only pinned section in the system, and only on desktop pointers. A second pin, or any pin on touch, is out of system.
...
- **Do** gate every scrubbed, parallaxed or pinned effect behind the desktop media query, and give touch a one-shot reveal instead.
...
- **Don't** add a second pinned section, run scrubbed transforms on touch, or let Lenis take over scrolling on a coarse pointer.
