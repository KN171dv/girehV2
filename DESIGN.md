...
The rebuilt site is editorial and large-format. The type ramp runs from 0.78rem captions to a 6.5rem hero in fluid `clamp()` steps, so the display voice actually dominates its viewport instead of politely fitting. Compositions bleed past the container and overlap on purpose — at desktop widths the manifesto figure runs off the right edge, the craft scene stacks two absolutely positioned figures, the gallery is a horizontal strip of mismatched widths. There are no cards: no bordered boxes around images, no shadowed containers, no padded tiles. Depth comes from tonal layering, gradient veils and overlap.
...
One container for the whole site: `max-w-[92rem]`, centred, with gutters stepping 1.5rem (base) → 2.5rem (sm) → 3.5rem (lg). Vertical rhythm is 5rem (base) → 8rem (sm) → 9rem (lg) per section, with the two most editorial sections (Manifesto, Trust) opening to 10rem at lg and the footer's data grid running 5rem/6rem. Anchored sections carry `scroll-margin-top: 5rem` so the fixed header never covers a target.

The base step is 5rem, not 6rem, because a phone measures section breaks against a viewport of roughly 844px rather than against the whole canvas: at 6rem the two paddings met as 192px of nothing, close to a quarter of the screen, and the page read as scattered elements rather than as a composition. Trust is the one asymmetric section, opening at 3.5rem against a 5rem close, because its gold hairline is the divider between it and the gallery and has to sit near what precedes it to read as a transition. Measured gaps between the last ink of one section and the first ink of the next, at 390px: 80 / 87 / 96 / 160 / 144 / 185 / 80. The wider ones are the transitions that also change tonal band, where the break is the point; the tighter ones are where a photograph bleeds into the padding and closes the section by itself.

Composition is a 12-column grid at lg that is asymmetric by default and deliberately broken at the edges. The manifesto runs text in columns 1–7 with its figure in 8–12 bleeding past the gutter (`lg:-mr-14`). That bleed exists only at lg and above, where there is a grid gutter to break; below lg there is no grid, so the figure sits at the column width with equal margins like every other stacked photograph, and its signature row — logo, rule, "Desde 2015" — centres as one piece, because the third column that balances that row from the right (`Rio das Ostras / RJ`) is hidden below sm and its absence left the pair stranded against the left edge (both corrected 2026-09-21 on client report). The craft section abandons flow at xl entirely: a `min-h-[46rem]` stage with a 41%-wide figure anchored left, text anchored right, and a second 28%-wide figure absolutely placed at `left-[24%]` crossing over the first, separated from it by a 10px grafite border. Below xl the section is a vertical sequence — figure, text, figure — in two distinct forms. From md to lg the second figure is right-aligned at 46% and the first pulls back to 84%, so the two read as offset plates at different horizontal positions rather than as one crossing the other. Below md it changes shape entirely: the closing figure becomes a 16/10 band at the full column width, because a 64%-wide square pinned right was the desktop plate shrunk — it left a dead wedge beside it and made the gap to the next section read as roughly double its measured size. The band stays inside the column and does not bleed. An earlier version ran it to the screen edge; the 24px it then gained over the large figure directly above read as a strip of another photograph escaping past the side of the screen, because it broke the one alignment line the rest of the page keeps (corrected 2026-09-20 on client report). The threshold sits at xl, not lg, because 1024px is a tablet in landscape and a phone in desktop-site mode: there the crossing has no room and reads as a layout collision rather than as depth (corrected 2026-09-19 on client report). The services section pairs a sticky image panel (columns 1–5, `top-28`) with the ledger (columns 6–12). The barber roster is a 1 → 2 → 4 column grid whose four columns carry staggered top offsets (0, 5rem, 2rem, 7rem) so the row never reads as a card shelf. The gallery is a full-bleed horizontal strip of six panels, four aspect ratios and per-panel vertical offsets (±1.5rem–4rem), sized at lg by `min(vw, svh)` so it fits short screens.
...
### Shadow Vocabulary
- **Overlap-cast** (`box-shadow: 0 30px 70px -30px rgba(0,0,0,0.75)`, xl only): the single soft cast under the craft section's overlapping second figure, present only where one image physically crosses another. It reads as contact between layers, not as a container. Below xl there is no crossing, so there is neither cast nor border — the stacked state carries no frame of any kind.
...
> Known defect, open as of 2026-09-19: this rule does not hold reliably. `.impeccable/scripts/resize-anchor.cjs` reports 1–2 failures per run, where the page lands back at the top instead of at the anchor, and the section it happens to hit varies between runs. Verified pre-existing by A/B against the previous commit at the same failure rate, so it is not caused by the craft-composition change of the same date. Scheduled for its own round; do not treat the rule as guaranteed until that lands.
...
- **Do** keep every photograph in a stacked mobile section on the same alignment line — the column, 24px in from each screen edge. Below md the page holds one vertical line and the eye reads any deviation from it as a leak, not as a decision. A full-bleed photograph is still allowed there, but only where it runs edge to edge as a band of its own (the hero, the contact facade), never where it gains a few pixels over another photograph in the same section.
